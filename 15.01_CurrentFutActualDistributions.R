...
msgs  <- vapply(results, `[[`, character(1), "msg")
spnms <- vapply(results, `[[`, character(1), "species")

cat("\nDone.\n")
cat("Successful:", sum(ok), " / ", length(ok), "\n")
if (any(!ok)) {
  cat("\nFailures:\n")
  print(data.frame(species = spnms[!ok], msg = msgs[!ok]))
}
