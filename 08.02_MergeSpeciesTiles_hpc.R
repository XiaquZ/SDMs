...
# --- Config ---
in_dir    <- "/lustre1/scratch/348/vsc34871/SDM_fut/results/"
out_dir   <- "/lustre1/scratch/348/vsc34871/SDM_fut/Species_final/"
...
for (sp in names(groups)) {
  ...
  r <- try(
    vrt(sp_files, filename = vrt_file, overwrite = TRUE),
    silent = TRUE)
  ...
  out_tif <- file.path(out_dir, paste0(sp, "_3035.tif"))
